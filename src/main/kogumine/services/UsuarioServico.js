...
class UsuarioServico {
      ...
      if (result) {
        ...
        try {
          const resultado = await query(
            'INSERT INTO colecao (idUsuario,custoTotal,qtd_mythic,qtd_rare,qtd_common,totalCards,qtd_uncommon) VALUE (?,0,0,0,0,0,0)',
            foreignKey
          )
  ...
  static async putUsuario(usuario, id) {
    ...
    const sqlUpdate =
      'UPDATE usuarios SET username = ?, senha = ?, email = ? WHERE id = ?'
    try {
  ...
  static async autenticar(email, senha) {
    ...
    const auth =
      'SELECT usuarios.id,colecao.id AS idColecao FROM usuarios JOIN colecao ON colecao.idUsuario = usuarios.id WHERE email = (?) AND senha = (?)'
