...
class ColecaoServico{
  ...
  // }
    ...
    static async colecaoSUPERData(idColecao){
      ...
      const totalCards = await query(`SELECT SUM(qtdCarta) FROM carta_colecao WHERE idColecao = ${idColecao}`)
      const colecaoCartas = await query(`SELECT * FROM (SELECT idCarta,qtdCarta FROM carta_colecao WHERE idColecao = ${idColecao}) Res1 LEFT JOIN carta on Res1.idCarta = carta.id`)
      let precoTotal =  0
      ...
      arrayofcoiso.forEach(async(element) => {
        const rarityCard = await query(`SELECT SUM(qtdCarta) FROM (SELECT * FROM (SELECT idCarta,qtdCarta FROM carta_colecao WHERE idColecao = ${idColecao}) Res1 LEFT JOIN carta on Res1.idCarta = carta.id) result WHERE result.raridade = '${element}' `)
        ...
        if(cardTP != null){
          ...
          await query(`UPDATE colecao SET ${raridadeString} = ${cardTP} WHERE id = ${idColecao}`)
        }else{
          raridadeString += element

          await query(`UPDATE colecao SET ${raridadeString} = 0 WHERE id = ${idColecao}`)
        }
      })
      ...
      await query(`UPDATE colecao SET custoTotal = ${precoTotal} , totalCards = ${numCards} WHERE id = ${idColecao}`)
    }
